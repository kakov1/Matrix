...
private:
  ...
  public:
    ...
    const T &operator[](std::size_t idx) const { return str_ptr_[idx]; }
  };
...
private:
  ...
  template <typename It> void validate_correct_it(const It &it, const It &fin) {
    if (it == fin) {
      throw std::invalid_argument("Incorrect size for given matrix.");
    }
  }
...
public:
  ...
  Matrix(int rows, int cols, It start, It fin) : MatrixBuf<T>(rows * cols) {
...
template <typename T> int partial_pivoting(Matrix<T> &mtrx, int col) {
  int pivot = col;
  ...
  for (int i = col + 1; i < rows; ++i) {
    if (is_less(std::fabs(mtrx[pivot][col]), std::fabs(mtrx[i][col]))) {
      pivot = i;
    }
  }
...
template <typename T> T triangular_view_bareiss(Matrix<T> mtrx) {
  int sign = 1;
  T prev = 1;
  ...
  for (int i = 0; i < rows - 1; ++i) {
    ...
    for (int j = i + 1; j < rows; ++j) {
      ...
      for (int k = i + 1; k < rows; ++k) {
        T numerator = (mtrx[j][k]) * (mtrx[i][i]) - (mtrx[j][i]) * (mtrx[i][k]);
...
template <typename T> T triangular_view_gauss(Matrix<T> mtrx) {
  int sign = 1;
